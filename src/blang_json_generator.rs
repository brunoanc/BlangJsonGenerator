...
fn parse_blang(blang_bytes: Vec<u8>) -> Vec<BlangString> {
    ...
    for _i in 0..string_amount {
        ...
        let identifier = str::from_utf8(&blang_bytes[pos..(pos + identifier_len)]).unwrap().to_string();
        pos += identifier_len;

        let text_len = i32::from_le_bytes(blang_bytes[pos..(pos + 4)].try_into().unwrap()) as usize;
        pos += 4;

        let text = str::from_utf8(&blang_bytes[pos..(pos + text_len)]).unwrap().to_string();
        pos += text_len;
...
fn generate_blang_json(path: String, internal_path: String, out_path: String) {
    ...
    let blang_strings = parse_blang(blang_bytes);
...
fn modified_blang_to_json(modified_path: String, vanilla_path: String, internal_path: String, out_path: String) {
    ...
    let mut result = panic::catch_unwind(|| {
        parse_blang(vanilla_blang_bytes.clone());
    });
    if result.is_err() {
    ...
    let vanilla_strings = parse_blang(vanilla_blang_bytes);
    ...
    result = panic::catch_unwind(|| {
        parse_blang(modified_blang_bytes.clone());
    });
    ...
    let modified_strings = parse_blang(modified_blang_bytes);
